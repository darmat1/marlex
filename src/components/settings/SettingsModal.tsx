...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose }) => {
  ...
  const [updateStatus, setUpdateStatus] = useState<{ hasUpdate: boolean; msg: string; latestVersion?: string; url?: string } | null>(null);
  ...
  const handleCheckUpdate = async () => {
    ...
    try {
      ...
      if (info && info.hasUpdate) {
        setUpdateStatus({
          hasUpdate: true,
          msg: `Найдена новая версия v${info.latestVersion}! (текущая: v${info.currentVersion})`,
          latestVersion: info.latestVersion,
          url: info.downloadUrl || info.htmlUrl,
        });
      } else {
        setUpdateStatus({
          hasUpdate: false,
          msg: `У вас установлена самая последняя версия (v${info.currentVersion}).`,
        });
      }
    } catch (e: any) {
      setUpdateStatus({
        hasUpdate: false,
        msg: `Не удалось проверить обновления: ${e.message || e}`,
      });
    } finally {
  ...
  // If in web/iPad, enforce BYOK mode automatically
  useEffect(() => {
    if (!inElectron && llmConfig.mode !== 'byok') {
      setLLMConfig({ mode: 'byok' });
    }
  }, [inElectron, llmConfig.mode, setLLMConfig]);
    ...
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 select-none">
      ...
      >
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-zinc-400">
                {inElectron ? 'Codex / ChatGPT, Claude Code, Gemini CLI, Ollama (без API-ключей)' : 'OpenAI GPT-4o, Anthropic Claude 3.7, Google Gemini 2.0'}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 flex flex-col gap-5 max-h-[75vh] overflow-y-auto">
          ...
          {/* Mode Switcher (Visible only in Desktop Electron App) */}
          {inElectron && (
          ...
          {/* LOCAL CLI SECTION (Desktop Only) */}
          {inElectron && llmConfig.mode === 'local_cli' ? (
          ...
          {/* App Version & Updates Section (Desktop Electron Only) */}
          {inElectron && (
            <div className="pt-4 border-t border-zinc-800 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-xl bg-zinc-800 hover:bg-zinc-700 text-xs font-semibold text-zinc-200 border border-zinc-700 flex items-center gap-1.5 transition-colors disabled:opacity-50 cursor-pointer"
                >
              ...
              {updateStatus && (
                <div className={`p-3 rounded-xl border text-xs leading-relaxed flex items-center justify-between ${
                  ...
                  {updateStatus.hasUpdate && updateStatus.url && (
                    <button
                      onClick={() => openDownloadUrl(updateStatus.url)}
                      className="px-3 py-1 rounded-lg bg-amber-500 hover:bg-amber-400 text-zinc-950 font-bold text-[11px] flex items-center gap-1 shadow-sm cursor-pointer ml-3 shrink-0"
                    >
                      <Download className="w-3 h-3 stroke-[2.5]" />
                      <span>Скачать v{updateStatus.latestVersion}</span>
                    </button>
                  )}
                </div>
              )}
